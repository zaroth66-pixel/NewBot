...
| Item | Details |
|------|---------|
| Create Task Endpoint | `POST https://app-d1knsggu3vgh-api-GYX1bbkRQj4a.gateway.appmedo.com/v1/images/editing/expand` |
| Query Task (Single) Endpoint | `GET https://app-d1knsggu3vgh-api-AalZkkAG5w7L.gateway.appmedo.com/v1/images/editing/expand/{task_id}` |
| Query Task (List) Endpoint | `GET https://app-d1knsggu3vgh-api-pLVzAAkGZwDL.gateway.appmedo.com/v1/images/editing/expand` |
...
async function createExpandTask(params: {
  ...
  const response = await fetch(
    "https://app-d1knsggu3vgh-api-GYX1bbkRQj4a.gateway.appmedo.com/v1/images/editing/expand",
    {
      method: "POST",
      headers: {
...
async function queryExpandTask(taskId: string): Promise<{
  ...
  const response = await fetch(
    `https://app-d1knsggu3vgh-api-AalZkkAG5w7L.gateway.appmedo.com/v1/images/editing/expand/${taskId}`,
    {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
