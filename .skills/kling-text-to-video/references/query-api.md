...
| Property | Value |
|----------|-------|
| Endpoint | `GET https://app-d1knsggu3vgh-api-oLpZ7eD5j2Pa.gateway.appmedo.com/v1/videos/text2video/{id}` |
| HTTP Method | `GET` |
...
async function queryTask(taskId: string): Promise<QueryTaskResult> {
  const response = await fetch(
    `https://app-d1knsggu3vgh-api-oLpZ7eD5j2Pa.gateway.appmedo.com/v1/videos/text2video/${encodeURIComponent(taskId)}`,
    {
      method: "GET",
      headers: {
