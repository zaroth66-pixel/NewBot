...
| Property | Value |
|----------|-------|
| Endpoint | `GET https://app-d1knsggu3vgh-api-Xa6JZq2055oa.gateway.appmedo.com/search` |
...
```json
{
  ...
  "pagination": {
    "current": 1,
    "next": "https://app-d1knsggu3vgh-api-Xa6JZq2055oa.gateway.appmedo.com/search?start=10"
  }
}
```
...
async function searchGoogleScholar(params: ScholarSearchParams): Promise<ScholarResponse> {
  ...
  const response = await fetch(
    `https://app-d1knsggu3vgh-api-Xa6JZq2055oa.gateway.appmedo.com/search?${queryParams.toString()}`,
    {
      method: "GET",
      headers: {
